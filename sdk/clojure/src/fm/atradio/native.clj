...
(defn resolve-lib
  ...
  []
  (let [ext (os-ext)
        local (io/resource (str "libatradio_uniffi." ext))]
    (if local
      (.getAbsolutePath (File. (.toURI local)))
      (let [m (json/read-str (slurp (io/resource "atradio/manifest.json")))
            t (triple)
            sha (get-in m ["checksums" t])
            _ (when-not sha
                (throw (ex-info (str "no prebuilt native lib for " t
                                     " (manifest has no checksum) — run ./build.sh")
                                {:triple t})))
            tag (get m "tag")
            dest (File. (str (cache-dir tag) "/libatradio_uniffi-" t "." ext))]
