...
# Ruby, and Clojure SDKs from the release artifacts (libatradio_uniffi-<triple>.<ext>).
...
python3 - "$dir" "$tag" "$repo" \
  "$sdk/python/src/atradio/manifest.json" \
  "$sdk/ruby/lib/manifest.json" \
  "$sdk/clojure/resources/atradio/manifest.json" <<'PY'
import sys, os, glob, json, hashlib
d, tag, repo, *outs = sys.argv[1:]
checks = {}
for lib in sorted(glob.glob(os.path.join(d, "libatradio_uniffi-*"))):
    triple = os.path.basename(lib)[len("libatradio_uniffi-"):].rsplit(".", 1)[0]
    checks[triple] = hashlib.sha256(open(lib, "rb").read()).hexdigest()
if not checks:
    sys.exit(f"no libatradio_uniffi-* files in {d}")
