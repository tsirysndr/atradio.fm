...
Order of preference:
  ...
  2. A checksum-verified copy in the user cache, downloaded on first load (the
     published-wheel path: the ~11 MB lib is not bundled in the pure-Python wheel).
...
generated `atradio_uniffi.py` loader is patched by build.sh to call resolve().
"""
...
def resolve() -> str:
    ...
    ext = _ext()
    local = _HERE / f"libatradio_uniffi.{ext}"
    if local.exists():
        return str(local)
    ...
    if not sha:
        raise RuntimeError(
            ...
            f"run ./build.sh for a local build"
        )
    tag = manifest["tag"]
    dest = _cache_dir(tag) / f"libatradio_uniffi-{triple}.{ext}"
    if dest.exists() and _sha256(dest.read_bytes()) == sha:
        return str(dest)

    url = (
        f"https://github.com/{manifest['repo']}/releases/download/"
        f"{tag}/libatradio_uniffi-{triple}.{ext}"
    )
